...
class McubeWebhookService
{
    ...
    public function process(array $payload): array
    {
        try {
            ...
            // 2. Find agent by emp_phone
            $empPhone = $this->normalizePhone($payload['emp_phone'] ?? '');
            $agent    = $this->findUserByPhone($empPhone);
            if (!$agent) {
                return $this->result('failed', "Agent not found for phone: {$empPhone}");
            }
            ...
            $msg = $leadCreated
                ? "New lead created and call logged. Agent: {$agent->name}"
                : "Existing lead updated. Agent: {$agent->name}";
